...
EFI_STATUS
EFIAPI
EmmcGetDeviceState(IN MMC_HOST_INSTANCE *MmcHostInstance,
...
EFI_STATUS
EFIAPI
EmmcSetEXTCSD(IN MMC_HOST_INSTANCE *MmcHostInstance, UINT32 ExtCmdIndex,
              UINT32 Value) {
  EFI_MMC_HOST_PROTOCOL *Host;
...
EFI_STATUS
EFIAPI
EmmcIdentificationMode(IN MMC_HOST_INSTANCE *MmcHostInstance,
  ...
  Status =
      Host->ReceiveResponse(Host, MMC_RESPONSE_TYPE_R2,
  ...
  MmcHostInstance->CardInfo.RCA =
      ++mEmmcRcaCount; // TODO: might need a more sophisticated way of doing
                       // this
  RCA = MmcHostInstance->CardInfo.RCA << RCA_SHIFT_OFFSET;
  ...
  if (MMC_HOST_HAS_SETIOS(Host)) {
    ...
    Status =
        EmmcSetEXTCSD(MmcHostInstance, EXTCSD_BUS_WIDTH, EMMC_BUS_WIDTH_1BIT);
    if (EFI_ERROR(Status)) {
      DEBUG(
          (DEBUG_ERROR,
  ...
  Status = Host->ReadBlockData(Host, 0, 512,
  ...
  // Set up media
  Media->BlockSize =
      EMMC_CARD_SIZE; // 512-byte support is mandatory for eMMC cards
  ...
  Media->LastBlock = MmcHostInstance->CardInfo.ECSDData->SECTOR_COUNT -
...
FreePageExit:
  FreePages(MmcHostInstance->CardInfo.ECSDData,
...
STATIC
EFI_STATUS
InitializeEmmcDevice(IN MMC_HOST_INSTANCE *MmcHostInstance) {
  EFI_MMC_HOST_PROTOCOL *Host;
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR,
           "InitializeEmmcDevice(): Failed to switch high speed mode, "
           "Status:%r.\n",
           Status));
    return Status;
  }
  ...
  for (Idx = 0; Idx < 4; Idx++) {
    ...
    if (!EFI_ERROR(Status)) {
      ...
      if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR,
               "InitializeEmmcDevice(): Failed to set EXTCSD bus width, "
               "Status:%r\n",
               Status));
      }
...
STATIC
UINT32
CreateSwitchCmdArgument(IN UINT32 Mode, IN UINT8 Group, IN UINT8 Value) {
  UINT32 Argument;
...
STATIC
EFI_STATUS
InitializeSdMmcDevice(IN MMC_HOST_INSTANCE *MmcHostInstance) {
  UINT32 CmdArg;
  UINT32 Response[4];
  UINT32 Buffer[128];
  UINT32 Speed;
  ...
  if (MmcHostInstance->CardInfo.CardType == SD_CARD_2_HIGH) {
    ...
    NumBlocks =
        (CardSize + 1) * (1 << (MMC_CSD_GET_DEVICESIZEMULT(Response) + 2));
    BlockSize = 1 << MMC_CSD_GET_READBLLEN(Response);
  }
  ...
  if (CccSwitch) {
    ...
    if (EFI_ERROR(Status)) {
      ...
      Status =
          MmcHost->ReadBlockData(MmcHost, 0, SWITCH_CMD_DATA_LENGTH, Buffer);
      if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR,
    ...
    if (!(__be32_to_cpu(Buffer[3]) &
          SD_HIGH_SPEED_SUPPORTED)) { /* todo fixme bigendian cpu is no op */
      ...
      if (EFI_ERROR(Status)) {
        ...
        if ((__be32_to_cpu(Buffer[4]) & SWITCH_CMD_SUCCESS_MASK) !=
            0x01000000) {
          DEBUG((DEBUG_ERROR,
...
EFI_STATUS
EFIAPI
MmcIdentificationMode(IN MMC_HOST_INSTANCE *MmcHostInstance) {
  EFI_STATUS Status;
  UINT32 Response[4];
  UINTN Timeout;
  UINTN CmdArg;
  ...
  if (Status == EFI_SUCCESS) {
    if (!OcrResponse.Ocr.PowerUp) {
      DEBUG((DEBUG_ERROR,
             "MmcIdentificationMode(MMC_CMD1): Card initialisation failure, "
             "Status=%r.\n",
             Status));
      return EFI_DEVICE_ERROR;
    }
  ...
  // Check which kind of card we are using. Ver2.00 or later SD Memory Card
  // (PL180 is SD v1.1)
  ...
  if (Status == EFI_SUCCESS) {
    ...
    if (EFI_ERROR(Status)) {
      DEBUG((DEBUG_ERROR,
             "MmcIdentificationMode() : Failed to receive response to CMD8, "
             "Status=%r.\n",
             Status));
      return Status;
    }
  ...
  // We need to wait for the MMC or SD card is ready =>
  // (gCardInfo.OCRData.PowerUp == 1)
  Timeout = MAX_RETRY_COUNT;
  while (Timeout > 0) {
    // SD Card or MMC Card ? CMD55 indicates to the card that the next command
  ...
  //
  // Note, SD specifications say that "if the command execution causes a state
  // change, it will be visible to the host in the response to the next command"
...
EFI_STATUS
InitializeMmcDevice(IN MMC_HOST_INSTANCE *MmcHostInstance) {
  EFI_STATUS Status;
  EFI_MMC_HOST_PROTOCOL *MmcHost;
  UINTN BlockCount;
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR,
           "InitializeMmcDevice(MMC_CMD16): Error "
